"""Lightweight per-user dictation event bus for SSE."""
...
def publish_event(username: str, event_type: str, data: dict[str, Any] | None = None) -> None:
...
def wait_for_events(username: str, cursor: int, timeout: float = 5.0) -> tuple[list[dict[str, Any]], int]:
    buf = _get_buffer(username)
    with buf.cond:
